...
int
HPCC_RandomAccess(HPCC_Params *params, int doIO, int *failure) {
  u64Int i;
  u64Int temp;
...
void main(int argc, char** argv){
  ...
  HPCC_Params riscv;
  riscv.HPLMaxProcMem=134217728;// 521MB
  printf("HPLMaxProcMem=%d\n", (int)riscv.HPLMaxProcMem);
  //char name[]={'/','h','o','m','e','/','x','h','c','/','r','i','s','c','v','_','m','e','m','t','e','s','t','\0'};
  riscv.outFname = argv[1];
  printf("outputfilename=%s.\n",argv[1]);
 
  int doIO =1;
  int failure;
  printf("doIO=%d\nStart simulating!\n",doIO);
  HPCC_RandomAccess(&riscv, doIO, &failure);
  printf("goodbye!\n");
